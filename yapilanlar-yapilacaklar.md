...
- [x] **"Hakkımızda" sayfası eklendi (2026-09-01, deploy edildi, canlıda doğrulandı):** `/hakkimizda/` + `/en/about/`. İçerik: ne olduğumuz, kaynak seçim ilkelerimiz (hakemli/preprint ayrımı, tartışmalı bulgularda dengeli sunum — Blue Zones/Saul Newman örneği), kim yürütüyor (kişisel isim/uydurma unvan yok — mevcut KVKK metninin "sadece marka adı" yaklaşımıyla tutarlı), sınırlarımız, iletişim (salihzekipehlivan@gmail.com, koddaki mevcut KVKK iletişim adresiyle aynı). AboutPage+BreadcrumbList JSON-LD, hreflang karşılıklı. Footer'a link eklendi (TR+EN ana sayfa), `sitemap.xml` ve `llms.txt` güncellendi. `curl` ile canlıda 200 doğrulandı.
